...
- [x] Copier le projet sur kube-1 via `scp`
- [x] Déployer **nginx-ingress** sur le nœud `ingress`
- [x] Déployer **kube-prometheus-stack + Grafana** sur le nœud `monitoring`
- [x] Déployer **Loki** sur le nœud `monitoring`
- [x] Déployer le **Kubernetes Dashboard**
- [x] Builder et pusher l'image Docker de l'application Laravel
- [x] Déployer l'application via le **Helm chart**
- [x] Configurer les **Ingress** (règles de routage HTTP)
...
# ingress
ssh -i ~/.ssh/kubequest.pem ec2-user@108.130.65.49
```
